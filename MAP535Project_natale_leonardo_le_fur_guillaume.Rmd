...
We still have way too many features for the number of observations we have. (`r dim(train.modeled)`). We will need to do some variable selection afterwards to reduce the number of predictors.
...
```{r Breusch Pagan}
res.bp.test <- ols_test_breusch_pagan(res.full.lm)
```

The p-value of the Breusch-Pagan test is `r res.bp.test$p`. Consequently, we reject the homoskedasticity hypothesis.
...
the third plot shows now some improvement regarding homoscedasticity. We could work with this model instead.

```{r Breausch Pagan 2}
res.bp.test2 <- ols_test_breusch_pagan(res.full.log.lm) 
```

Even though it looks better, the p-value of the Breusch-Pagan test on this model is `r res.bp.test2$p`. It is a bit higher but we still reject the homoskedasticity hipothesis.
...
We want to determine wether there are outliers in our data.

```{r InfluenceIndexPlot}
car::influenceIndexPlot(res.full.lm, vars=c("Cook", "Studentized", "hat"))
```
...
The index of the outliers pointed out by the test are `r outliers.index`
