...
```{r}
res.caret.lambda <- caret::train(
  x = train.modeled,
  y = train.y,
  method = "glmnet",
  tuneGrid = data.frame(
      alpha = 0,
      lambda = seq(2e4, 4e4, 100)
  ),
  metric = "RMSE"
)
res.caret.alpha <- caret::train(
  x = train.modeled,
  y = train.y,
  method = "glmnet",
  tuneGrid = data.frame(
      alpha = seq(0, 0.03, 0.002),
      lambda = res.caret.lambda$bestTune$lambda
  ),
  metric = "RMSE"
)
```
...
```{r}
plot(res.caret.lambda)
res.caret.lambda$bestTune
plot(res.caret.alpha)
res.caret.alpha$bestTune
```
