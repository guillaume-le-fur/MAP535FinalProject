...
We also created new features based on aggregations of the response variable by some of the predictors. We computed the average SalePrice per : Neighborhood, LotShape, OverallCond and MSSubClass.
...
But even though this method allowed us to reduce significantly our number of columns, we still have to many columns to plot a correlation matrix. Therefore, we display only correlations that are above the value `r cor.threshold`.
...
This project was a good opportunity to put what we have learnt this semester into practice and also to become more at ease with R.
